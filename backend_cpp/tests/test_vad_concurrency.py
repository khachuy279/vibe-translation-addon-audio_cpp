...
def test_vad_engine_factory_unknown_name_falls_back_to_the_default():
    """An unknown engine name must resolve to the DEFAULT engine, not the priciest one.

    The fallback used to be a hard-coded ``firered-vad`` -- the most expensive engine
    (~8x silero) -- so a typo in the VAD config silently cost CPU. Measured on real speech,
    firered-vad is 108.7 ms/audio-sec versus 57.9 for fsmn-vad.

    Identity is asserted rather than cost, because cost is noisy while the fallback target is
    ...
    VADEngineFactory.reset_pool()
    try:
        default_engine = VADEngineFactory.get_engine(app_config.vad.vad_engine)
        fallback = VADEngineFactory.get_engine("definitely-not-an-engine")
        empty = VADEngineFactory.get_engine("")

        assert fallback is default_engine
        assert empty is default_engine
        assert app_config.vad.vad_engine == "fsmn-vad", (
            "default engine changed; re-run scratch/compare_vad_quality.py and update the "
            "cost table in backend_cpp/config.py before accepting a new default"
        )
    finally:
        VADEngineFactory.reset_pool()
