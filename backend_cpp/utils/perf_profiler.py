...
class MetricsCollector:
    ...
    def __init__(self, max_samples: int = 1000):
        ...
        # Leak detection state -- see record_resource_checkpoint().
        self._leak_threshold_mb: float = 500.0
        self._leak_baseline_warm: bool = False
    ...
    def reset(self) -> None:
        ...
        with self._lock:
            ...
            self._baseline_resources = self.get_resource_snapshot()
            # A fresh baseline means the process must warm up again before growth can be
            # called a leak. See record_resource_checkpoint().
            self._leak_baseline_warm = False
    ...
    def get_samples(self, category: str, name: str) -> List[float]:
        """Return a snapshot of the raw samples recorded for one metric.

        Samples are appended in recording order, so two metrics that are recorded together
        (for example ``asr.infer_ms`` and ``asr.audio_dur_sec``) stay index-aligned and can
        be zipped by benchmark tooling to correlate cost against input size.
        """
        with self._lock:
            return list(self._samples.get(category, {}).get(name, ()))
    ...
    def record_resource_checkpoint(self, checkpoint_name: str) -> Dict[str, Any]:
        ...
        Leak detection must not confuse a one-time model load with a leak.

        ``_baseline_resources`` is captured before any model is touched, and GGUF weights
        are mapped in only on first use, so the FIRST checkpoint after real work legitimately
        jumps by hundreds of MB. Measured on the W2.1 real-audio scenario:

        * run 1: +647.6MB  (lazy load of ASR + translation models)
        * run 2:   +6.9MB
        * run 3:   +5.5MB  (bounded allocator warm-up, not accumulation)

        Alerting on run 1 therefore fires on an *always-green* signal, which is worse than no
        signal at all: it trains the reader to ignore the warning so a genuine leak later goes
        unnoticed. Instead the first oversized jump re-arms the baseline (the process is now
        considered warm) and only growth measured from that warm baseline raises an alert.
        """
        ...
        with self._lock:
            ...
            warm = self._leak_baseline_warm
            if not warm and delta_ram > self._leak_threshold_mb:
                # One-time model load: adopt the loaded state as the new baseline.
                self._baseline_resources = snap
                self._leak_baseline_warm = True
                snap["leak_baseline_rearmed"] = True

        if snap.get("leak_baseline_rearmed"):
            logger.info(
                "🌱 [PERF] Resource baseline re-armed after a one-time +%.0fMB jump "
                "(model load). Leak detection now compares against %.0fMB.",
                delta_ram,
                snap["ram_rss_mb"],
            )
        elif delta_ram > self._leak_threshold_mb:
            self._add_alert(
    ...
    def dump_report_file(self, filepath: str) -> None:
        ...
        # NOTE: `config` must be imported locally. The module-level __getattr__ only serves
        # attribute access on the module object (perf_profiler.config); it is NOT consulted
        # for plain global lookups inside functions, so a bare `config` here raised
        # NameError as soon as perf.enabled was turned on.
        from backend_cpp.config import config
