...
class VADProcessor:
    ...
    def __init__(
        ...
        if self.vad_engine not in DEFAULT_THRESHOLDS:
            # Do not fall back silently: an unknown name used to land on `firered-vad`, the
            # most expensive engine (~8x silero), so a config typo quietly cost CPU. It also
            # means a typo could not be distinguished from a deliberate choice in the logs.
            logger.warning(
                "Unknown VAD engine %r; falling back to 'fsmn-vad' (supported: %s)",
                self.vad_engine,
        ...
        self._frame_f32_buf: np.ndarray = np.empty(self._frame_samples, dtype=np.float32)
    ...
    def feed_chunk(self, pcm_data: bytes, capture_timestamp: float = 0.0) -> None:
        ...
        with self._lock:
            ...
            while buf_len - offset >= frame_size:
                ...
                if len(self._frame_f32_buf) != self._frame_samples:
                    self._frame_f32_buf = np.empty(self._frame_samples, dtype=np.float32)
                np.copyto(self._frame_f32_buf, samples_int16, casting='unsafe')
                del samples_int16
                self._frame_f32_buf /= 32768.0
                samples_float32 = self._frame_f32_buf
