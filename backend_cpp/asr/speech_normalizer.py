...
class SpeechNormalizer:
    ...
    def sanitize_input(pcm: np.ndarray) -> np.ndarray:
        ...
        if isinstance(pcm, np.ndarray) and pcm.dtype == np.float32 and pcm.flags.c_contiguous:
            if np.all(np.isfinite(pcm)):
                return np.clip(pcm, -1.0, 1.0)
            x = np.nan_to_num(pcm, nan=0.0, posinf=1.0, neginf=-1.0, copy=True)
            return np.clip(x, -1.0, 1.0, out=x)
        x = np.asarray(pcm, dtype=np.float32)
        if not np.all(np.isfinite(x)):
            x = np.nan_to_num(x, nan=0.0, posinf=1.0, neginf=-1.0, copy=True)
        else:
            x = x.copy()
        return np.clip(x, -1.0, 1.0, out=x)
